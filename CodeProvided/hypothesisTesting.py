...
- Amount of treatments to conduct before first prediction of Gaussian process: See line 137-139.
- Multiplier start value: See line 306-307.
- Stepwidth value: See line 202-203.
- Maximum times one treatment is conducted: See line 225-226. 
- Specify a location for the hypothesis: See line 344-345.
...
- Prior information on objective function: See line 165-168. If no noice is specified, comment these lines out.
- Noise of observations: See line 169 - 170. If no noice is specified, estimate the best you can.
- Whilch treatments should be conducted before first prediction of Gaussian process: See line 140-141.
- Certaintly with which a hypothesis can be accepted / rejected: See line 282-289.
----------------------------------------------
3. Note:
...
This code is specified for experiments seeking hypothesis testing.
'''
...
'''
This function evaluares if a given hypothesis can be accepted / rejected yet.
It return a decision or None.
'''
def evaluateHypothesis(hypothesis, allTrialsDictionary, m, forceChoice, multiply):
    ...
    for i in range(len(allTrialsDictionary)):
        ...
        if pred[0][0][0] < y_predMin:
            ...
            y_predMinSigma = pred[1][0][0]
            x_predMin = a[0]
    # A choice has to be made, when all trials have been conducted.
    if forceChoice == True:
        if list(x_predMin) == list(hypothesis):
            acceptReject = 'accept'
        else:
            acceptReject = 'reject'
        return acceptReject
    hypothesisPred = m.predict(np.array([hypothesis]))
    for i in range(len(allTrialsDictionary)):
        a = np.array([getCorrectDimesion(allTrialsDictionary, i)])
        pred = m.predict(a)
        sumVariance += (pred[1][0][0] * multiply * 2)
        if pred[0][0][0] - (pred[1][0][0] * multiply) <= hypothesisPred[0][0][0]:
            sumVarianceBelowMin += ((pred[1][0][0] * multiply) - (pred[0][0][0] - hypothesisPred[0][0][0]))
    variance = sumVarianceBelowMin/sumVariance
    ...
    if variance < 0.01:
        ...
        return acceptReject
    return acceptReject
    # Rejecting a hypotheses.
    if hypothesisPred[0][0][0] > y_predMin + (y_predMinSigma * multiply * 2):
        acceptReject = 'reject'
        return acceptReject
...
'''      
def adaptiveDesignOptimization(file, hypothesis):
    ...
    while unfinished:
        ...
        if nextTrialInfo[1] == 0 or nextTrialInfo[0] == None:
            acceptReject = evaluateHypothesis(hypothesis, allTrialsDictionary, m, True, multiply)
            sheetResults['A'+str(sheetResults.max_row+2)] = acceptReject
            data.save(file)
            ...
            return
        acceptReject = evaluateHypothesis(hypothesis, allTrialsDictionary, m, False, multiply)
...
# Add a location in the feature space for a hypothesis here.
hypothesis = np.array([4.888888889, 4.888888889])
# If your excel file is calles differently, adjust the name here.
simulation = adaptiveDesignOptimization('Experiment.xlsx', hypothesis)