...
- Amount of treatments to conduct before first prediction of Gaussian process: See line 138-140.
- Multiplier start value: See line 360-361.
- Stepwidth value: See line 203-204.
- Models have to be specified: See line 390-391.
- Maximum times one treatment is conducted: See line 320-321. 
...
- Prior information on objective function: See line 166-169. If no noice is specified, comment these lines out.
- Noise of observations: See line 170 - 171. If no noice is specified, estimate the best you can.
- Whilch treatments should be conducted before first prediction of Gaussian process: See line 140-141.
- Which model selection criteria should be used for the evaluation: The criteria needs to be taken out of the whole model selection process.
----------------------------------------------
3. Note:
...
This code is specified for experiments seeking model selection.
'''
...
'''
This function calculates the RSME and returns it value.
'''
def calculateRSME(X, Y, y):
    allDifference = 0
    for i in range(len(X)):
        difference = (Y[i] - y[i])**2
        allDifference += difference
    RSME = math.sqrt(allDifference)
    return RSME
    
'''
This function calculates R2 and returns it value.
'''
def calculateR2(X, Y, y):
    yMean = sum(y) / float(len(y))    
    print(yMean,'yMean')
    regressVariation, yVariation = 0, 0
    for i in range(len(X)):
        yHat = y[i][0]
        yCurrent = Y[i][0]
        regressVariation += (yHat - yMean)**2
        yVariation += (yCurrent - yMean)**2
    r2 = regressVariation / yVariation
    return r2
    
'''
This function calculates the AIC and returns it value.
'''
def calculateAIC(X, Y, y):
    sse = 0
    for i in range(len(X)):
        yHat = y[i][0]
        yCurrent = Y[i][0]
...
'''
This function calculates the BIC and returns it value.
'''
def calculateBIC(X, Y, y):
    sse = 0
    for i in range(len(X)):
        yHat = y[i][0]
        yCurrent = Y[i][0]
...
'''
This is a helper function to calculate a value from a model.
'''
def getYFromFunction(point, model):
    y = list(map(model, [point[0]],[point[1]]))    
    return y[0]

'''
This function evaluates which of the given models should be chosen. 
The model that is selected by more model selection crietria than any other model is selected.
The function returns the choice.
'''
def evaluateModelFit(X, Y, allTrialsDictionary, XVector, models):
    evaluationOfAllModels = []    
    # For each model selection criteria and each function calculate the value.    
    for i in range(len(models)):
        y = []        
        for j in range(len(XVector)):     
            y.append([getYFromFunction(XVector[j], models[i])])
        y = np.atleast_2d(y)    
        AIC, BIC, r2, RSME = calculateAIC(X, Y, y), calculateBIC(X, Y, y), calculateBIC(X, Y, y), calculateBIC(X, Y, y)
        evaluationOfAllModels.append([AIC, BIC, r2, RSME])
    AICEvaluationList, BICEvaluationList, r2EvaluationList, RSMEEvaluationList = [], [], [], []
    # For each model selection criteria, see which model is selected.
    for i in range(len(models)):
        AICEvaluationList.append(evaluationOfAllModels[i][0])        
        BICEvaluationList.append(evaluationOfAllModels[i][1])     
        r2EvaluationList.append(evaluationOfAllModels[i][2])     
        RSMEEvaluationList.append(evaluationOfAllModels[i][3])     
    AIC = AICEvaluationList.index(max(AICEvaluationList)) 
    BIC = BICEvaluationList.index(min(BICEvaluationList))
    r2 = r2EvaluationList.index(max(r2EvaluationList))
    RSME = RSMEEvaluationList.index(min(RSMEEvaluationList))
    # Choose the model that is selected by the most model selection criteria.
    try:
        results = mode([AIC, BIC, r2, RSME])
    except:
        results = AIC
    selectedModel = 'model '+str(results)
...
'''      
def adaptiveDesignOptimization(file, models):
    ...
    while unfinished:
        ...
        if nextTrialInfo[1] == 0 or nextTrialInfo[0] == None:
            selectedModel = evaluateModelFit(np.atleast_2d(XVector), np.atleast_2d(YVector), allTrialsDictionary, XVector, models)
            sheetResults['A'+str(sheetResults.max_row+2)] = selectedModel
            data.save(file)
...
# All models included need to be specified here. Following the structure of the models below.
models = [lambda x,y: x**2*y, lambda x,y: x**3*y, lambda x,y: x**4*y]
# If your excel file is calles differently, adjust the name here.
simulation = adaptiveDesignOptimization('Experiment.xlsx', models)
